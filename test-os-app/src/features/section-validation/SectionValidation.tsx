...
export function SectionValidation({
  ...
  useEffect(() => {
    setActiveSuiteId(initialActiveSuiteId)
  }, [initialActiveSuiteId])

  useEffect(() => {
    setActiveStoryId(initialActiveStoryId)
  }, [initialActiveStoryId])
  ...
  useEffect(() => {
    setPhaseComplete(initialSuites.length > 0 && initialSuites.every((s) => s.status === 'complete'))
  }, [initialSuites])
  ...
  const handleResetSuite = (suiteId: string) => {
    const shouldReset = window.confirm(
      'Reset this suite? This clears step completions, authored stories, fixtures, and screenshots for the suite.'
    )
