...
import type { DeploymentAndAutomationProps, DeploymentMode } from '@/../product/sections/deployment-and-automation/types'
...
export function DeploymentAndAutomation({
  ...
  const handleConfirmStep = (stepId: string) => {
    const updated = steps.map((s, i, arr) => {
      if (s.id === stepId) return { ...s, status: 'complete' as const, confirmedAt: new Date().toISOString() }
